...
permissions for \textit{user$\_$birthday} and \textit{publish$\_$stream}.
Data used by the quiz are stored in a MySQL-database, tables have the
following contents:
\begin{itemize}
...
participated or not.
\item \textbf{Friendships} - creates entries for each friendship found in
users' friend lists.
...
as correct or false.
\item \textbf{Given answers} - stores each answer given by a user.
\item \textbf{User score} - stores the score user had in the quiz.
\item \textbf{Correct answered questions} - stores which questions were correct
answered by which user.
\end{itemize}

The database queries are performed with the PHP
pear\footnote{http://pear.php.net/}-package. The script which performs
the queries uses few files: The index.php manages the application
flow. The library.php contains all the functions needed, and quiz.php,
question.php, and quizResult.php display the quiz as well as the
correct solution and the feature to invite friends.
